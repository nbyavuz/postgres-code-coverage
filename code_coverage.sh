...
get_baseline_coverage_file()
{
    ...
    run_lcov ${BASELINE_PG_DIR} ${BASELINE_COVERAGE_FILE}.base true

    run_tests_postgres ${BASELINE_PG_DIR}
    run_lcov ${BASELINE_PG_DIR} ${BASELINE_COVERAGE_FILE}.test false

    merge_lcov_files ${BASELINE_COVERAGE_FILE}.base ${BASELINE_COVERAGE_FILE}.test ${BASELINE_COVERAGE_FILE}
...
get_current_coverage_file()
{
    ...
    run_lcov ${CURRENT_PG_DIR} ${CURRENT_COVERAGE_FILE}.base true

    run_tests_postgres ${CURRENT_PG_DIR}
    run_lcov ${CURRENT_PG_DIR} ${CURRENT_COVERAGE_FILE}.test false

    merge_lcov_files ${CURRENT_COVERAGE_FILE}.base ${CURRENT_COVERAGE_FILE}.test ${CURRENT_COVERAGE_FILE}
